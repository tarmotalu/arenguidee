class UnsubscribesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @unsubscribe.save
        flash[:notice] = tr("You will no longer receive email updates from {instance_name}", "controller/unsubscribes", :instance_name => tr(current_instance.name,"Name from database"))
