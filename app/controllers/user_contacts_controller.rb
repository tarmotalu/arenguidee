class UserContactsController < ApplicationController
  ...
  def index
    @page_title = tr("Find people you know at {instance_name}", "controller/contacts", :instance_name => tr(current_instance.name,"Name from database"))
  ...
  def members
    @page_title = tr("Already members at {instance_name}", "controller/contacts", :instance_name => tr(current_instance.name,"Name from database"))
  ...
  def not_invited
    @page_title = tr("Not members yet, go ahead and invite them.", "controller/contacts", :instance_name => tr(current_instance.name,"Name from database"))
    @contacts = @user.contacts.active.not_members.not_invited.with_email
  end
  
  def invited
    @page_title = tr("People you've invited to join {instance_name}", "controller/contacts", :instance_name => tr(current_instance.name,"Name from database"))
  ...
  def new
    @page_title = tr("Invite people to join {instance_name}", "controller/contacts", :instance_name => tr(current_instance.name,"Name from database"))
      ...
      format.js {
        render :update do |page|
          ...
          if @user.contacts_not_invited_count == 0 # invited all their contacts
            flash[:notice] = tr("Thanks for inviting people to join {instance_name}. We'll send you an email when they join, and you'll earn 5{currency_short_name} too.", "controller/contacts", :currency_short_name => current_instance.currency_short_name, :instance_name => tr(current_instance.name,"Name from database"))
